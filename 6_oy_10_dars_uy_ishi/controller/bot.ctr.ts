...
export const getMessageLastTenDay = async (req: Request, res: Response, next: NextFunction,) => {
  try {
    const currentDate = new Date()
...
export const updateMessageDate = async (req: Request, res: Response, next: NextFunction,) => {
  try {
    const {id} = req.params
    const {msg} = req.body
